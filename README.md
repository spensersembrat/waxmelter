...
Personal eBay watchlist for sports cards. You save searches (watches). Scan now (or hourly, if official eBay keys exist) looks those up on eBay, compares each Buy It Now listing to that card's Card Ladder CL value (via Parse.bot), and alerts when CL is a set percent higher than the listing.
...
3. Set env vars: `SITE_PASSWORD`, `CRON_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `PARSE_API_KEY`
...
- `/comps` — Card Ladder CL values per watch
- `/login` — site password
